...
function generateStates(state: State): State[] {
  ...
  for (const apod of state.apods) {
    ...
    if (apod.row === 2 && apod.col === DESTINATION_BY_TYPE[apod.type]) {
      continue
    }

    if (apod.row === 1 && apod.col === DESTINATION_BY_TYPE[apod.type] && blockedPositions[2][apod.col] === apod.type) {
      continue
    }

    if (apod.row === 2 || apod.row === 1) {
      ...
      if (destination < apod.col) {
        for (let col = apod.col - 1; col >= 0; col--) {
          ...
          if (col === DESTINATION_BY_TYPE[apod.type]) {
            const otherApodType = blockedPositions[2][col]
            if (!otherApodType) {
              const newApods = [...otherApods, { type: apod.type, row: 2, col }]
              newStates.push({
                apods: newApods,
                energy: state.energy + ENERGY_BY_TYPE[apod.type] * (2 + apod.col - col),
                winning: checkForWin(newApods)
              })
            }
            if (otherApodType === apod.type && !blockedPositions[1][col]) {
              const newApods = [...otherApods, { type: apod.type, row: 1, col }]
              newStates.push({
                apods: newApods,
                energy: state.energy + ENERGY_BY_TYPE[apod.type] * (1 + apod.col - col),
                winning: checkForWin(newApods)
              })
            }
            break
        ...
        for (let col = apod.col + 1; col <= 10; col++) {
          ...
          if (col === DESTINATION_BY_TYPE[apod.type]) {
            ...
            if (!otherApodType) {
              ...
              newStates.push({
                apods: newApods,
                energy: state.energy + ENERGY_BY_TYPE[apod.type] * (2 + col - apod.col),
                winning: checkForWin(newApods)
              })
            }
            if (otherApodType === apod.type && !blockedPositions[1][col]) {
              ...
              newStates.push({
                apods: newApods,
                energy: state.energy + ENERGY_BY_TYPE[apod.type] * (1 + col - apod.col),
                winning: checkForWin(newApods)
              })
            }
            break
